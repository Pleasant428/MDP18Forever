...
public class Robot {
	...
	//Robot Variables
	private ArrayList<Sensor> sensorList;		//List of Sensors
	private boolean sim;						//Real or Sim Robot
	...
	public Robot(boolean sim, Direction dir, int row, int col) {
		...
		// Rotate the Sensors TO face the same direction as robot
		switch (dir) {
		case LEFT:
			rotateSensors(true);
			break;
	...
	public void rotateSensors(boolean left) {
		...
		for (Sensor s : sensorList) {
			if (left)
				s.setSensorDir(Direction.turnRight(s.getSensorDir()));
			else
				s.setSensorDir(Direction.turnLeft(s.getSensorDir()));

			newCol = (int) Math.round((Math.cos(angle) * (s.getCol() - pos.x) 
					- Math.sin(angle) * (s.getRow() - pos.y) + pos.x));
			
			newRow = (int) Math.round((Math.sin(angle) * (s.getCol() - pos.x) 
					- Math.cos(angle) * (s.getRow() - pos.y) + pos.y));
	...
	public void move(Direction dir, boolean forward, int steps, Map exploredMap) {
		...
		// Declare cells move through by robots on the exploredMap
	...
	//Update the starting position of robot
	...
	// Retrun Robot Position
	public Point getPosition() {
		return pos;
	}

	// Robot Sense method to get Sensor Data
	public void sense(Map exploredMap, Map map) {
		...
		if (!sim) {
			...
			//Loop till Sensor Data Received (Able to Recieve Image Detected)
			do {
				...
				if(msgArr[2].compareToIgnoreCase("S")!=0) {
					...
					if(obsDetected) {
						...
						exploredMap.detectedImg(loc, Direction.turnRight(direction));
						char c = Direction.turnRight(direction).name().charAt(0);
		...
		// Update Map Using All Data Recieved for every Sensor
		for (int i = 0; i < sensorList.size(); i++) {
			...
			for (int j = sensorList.get(i).getMinRange(); j <= sensorList.get(i).getMaxRange(); j++) {
				...
				if (exploredMap.checkValidCell(sensorList.get(i).getRow() + rowInc * j,
					...
					// Remove obstacles before the Sensor Reading of obstacles (Phantom Block Counter)
					else if (exploredMap
							.getCell(sensorList.get(i).getRow() + rowInc * j, sensorList.get(i).getCol() + colInc * j)
							.isObstacle()) {
						exploredMap.getCell(sensorList.get(i).getRow() + rowInc * j,
					...
					// Remove Obstacles after Sensor Reading of Obstacle (Phantom Block Counter)
		...
		// Check if Robot Requires Calibration and Calibrate if necessary before next move
		if (!sim) {
			
			// Checking Front Alignment too close/far from obstacles (Front Calibration)
			...
			// Check if too Close/Far from Right Wall
	...
	// Create and Send Map Descriptor to Android
